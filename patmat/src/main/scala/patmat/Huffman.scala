...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.map { c => (c, chars.count(findChr => findChr == c)) }.distinct
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.sortWith(_._2 < _._2).map { w => new Leaf(w._1, w._2) }
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.size >= 2) {
      //if there are may elements only combine
  ...
  /**
    ...
    */
  def until(condition: List[CodeTree] => Boolean, operation: List[CodeTree] => List[CodeTree])(tree: List[CodeTree])
  : List[CodeTree] = {
    if (condition(tree)) {
      tree
    } else {
